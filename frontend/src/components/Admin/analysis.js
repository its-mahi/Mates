...
export default function AdminAnalysis() {
 ...
 const[NoOfQuestions,setNoofquestion]=useState([])
    ...
    const noOfQuestions = async () => {
        await fetch('http://localhost:8000/api/admin/noOfQuestions', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const noOfusers =  async () => {
        await fetch('http://localhost:8000/api/admin/noOfUsers', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const noOfAnswers = async () => {
        await fetch('http://localhost:8000/api/admin/noOfAnswers', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const noOfAccept = async () => {
        await fetch('http://localhost:8000/api/admin/noOfAccept', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://localhost:8000/api/admin/question-by-month');
  ...
  useEffect(() => {
    async function fetchData() {
        try {
            const response = await fetch('http://localhost:8000/api/admin/question-by-year');
            const data = await response.json();
            data.forEach(item => {
